...
for j in range(len(data['Cidade'])):
    ...
    maxtemp[j]=max(np.mean(t2s[:27],axis=(1,2)))
    mintemp[j]=min(np.mean(t2s[:27],axis=(1,2)))
    acumpre[j]=np.mean(precs,axis=(1,2))[26]
    nebulos[j]=np.mean(clds[:27,:,:,:])
...
data.to_csv('{0:s}\\meteogram_omp_4km_{1:s}{2:s}.csv'.format(odir,date,hour))
